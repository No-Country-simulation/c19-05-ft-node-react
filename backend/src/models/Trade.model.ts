...
TradeSchema.pre('deleteOne', { document: true, query: false }, async function (next) {
	try {
		...
		const [memberOne, memberTwo] = await Promise.all([UserModel.findById(trade!.members.memberOne), UserModel.findById(trade!.members.memberTwo)]);
		memberOne!.trades = memberOne!.trades.filter(trade => trade?.toString() !== this.id.toString());
		memberTwo!.trades = memberTwo!.trades.filter(trade => trade?.toString() !== this.id.toString());
